...
export const defaultPageText: Record<string, string> = {
  ...
  bookingEyebrow: "BOOK A READING", bookingTitle: "看价格，然后来聊聊", bookingLead: "选一个大致方向，就能在这一页看完价格、档期和联系方式。不知道选哪个也没关系，直接来问就好。", bookingStep1Label: "第一步", bookingStep1Title: "选择服务类型", bookingEntryAction: "进入查看　↗", bookingSelectAction: "直接看价格　↓", bookingDreamEyebrow: "DREAM READING", bookingDreamTitle: "梦向解读", bookingDreamSummary: "梦占和传讯分别展示、分别计价。可以先看看两种服务的区别。", bookingRealityEyebrow: "REALITY CONSULTATION", bookingRealityTitle: "现实问题咨询", bookingRealitySummary: "感情、人际、学业、工作，或卡在某个选择里，都可以从这里开始。", bookingIdeasEyebrow: "CURIOUS CORNER", bookingIdeasTitle: "奇思妙想", bookingIdeasSummary: "OC、宠物、角色创作辅助，以及暂时不适合放进常规分类的题目。", bookingStep2Label: "第二步", bookingStep2Title: "准备问题", bookingPricingLabel: "第二步", bookingPricingTitle: "项目与价格", bookingPricingDetailAction: "查看详细说明", bookingAvailabilityLabel: "第三步 · 看看档期", bookingPrepareHint: "不知道怎么说也没关系，先发一句话就好；信息不够时我会再问。", bookingPrepare1Title: "问题类型", bookingPrepare1Text: "告诉我是梦向、现实问题，还是奇思妙想。", bookingPrepare2Title: "必要背景", bookingPrepare2Text: "有多少说多少，不需要提前整理成完整的小作文。", bookingPrepare3Title: "最想知道的事", bookingPrepare3Text: "先说最想弄明白哪一点，其他信息可以慢慢补。", bookingPromoLabel: "当前", bookingPromoTitle: "可用优惠", bookingContactLabel: "最后一步", bookingContactTitle: "联系不不", bookingContactReassurance: "可以先问是否适合，不需要马上决定。确认项目、价格和预计时间之后，再决定是否继续。", bookingBoundaryEyebrow: "BEFORE BOOKING", bookingBoundaryTitle: "接单范围", bookingBoundary1Label: "可以讨论：", bookingBoundary1Text: "情感、人际、学业、工作、阶段变化、梦角与梦设、OC及其他娱乐向主题。", bookingBoundary2Label: "暂不接：", bookingBoundary2Text: "生死、走失、重大疾病诊断、法律结论、投资或赌博决策。", bookingBoundary3Label: "服务说明：", bookingBoundary3Text: "卡牌作为图像、象征与叙事媒介，用于娱乐体验、角色创作和现实问题梳理；不提供通灵、改运或结果保证。", bookingBoundary4Label: "需要知道：", bookingBoundary4Text: "解读基于当前信息，现实条件变化后，结果也可能变化。",
...
async function ensureStorage() {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS site_owners (
      email TEXT PRIMARY KEY,
      claimed_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
    )`),
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS site_content_history (
...
export async function getSiteContent(): Promise<SiteContent> {
  try {
    ...
    const row = await db.prepare("SELECT payload FROM site_content WHERE id = 1").first<{ payload: string }>();
    if (!row?.payload) return defaultContent;
    const parsed = JSON.parse(row.payload) as Partial<SiteContent>;
    const content = cleanContent(parsed);
    if (content.contentVersion < CONTENT_VERSION) {
      const migrated = migrateTerminology(content);
      await db.prepare("UPDATE site_content SET payload = ?, updated_at = CURRENT_TIMESTAMP, updated_by = ? WHERE id = 1")
        .bind(JSON.stringify(migrated), "system:content-v15").run();
      return migrated;
    }
    if (JSON.stringify(parsed.availability?.overrides ?? []) !== JSON.stringify(content.availability.overrides)) {
      await db.prepare("UPDATE site_content SET payload = ?, updated_at = CURRENT_TIMESTAMP, updated_by = ? WHERE id = 1")
        .bind(JSON.stringify(content), "system:availability-cleanup").run();
    }
    return content;
  } catch {
...
export async function isOwner(email: string): Promise<boolean> {
  ...
  const owner = await db.prepare("SELECT email FROM site_owners WHERE email = ?").bind(normalized).first<{ email: string }>();
  return Boolean(owner);
}

export type ContentRevision = { id: string; createdAt: string; updatedBy: string };

export async function saveSiteContent(content: unknown, email: string): Promise<SiteContent> {
  const db = await ensureStorage();
  const editor = email.trim().toLowerCase();
  const clean = cleanContent(content);
  clean.contentVersion = CONTENT_VERSION;
  const current = await db.prepare("SELECT payload, updated_by FROM site_content WHERE id = 1").first<{ payload: string; updated_by: string }>();
  if (current?.payload) {
    await db.prepare("INSERT INTO site_content_history (payload, updated_by) VALUES (?, ?)").bind(current.payload, current.updated_by || editor).run();
    await db.prepare("DELETE FROM site_content_history WHERE id NOT IN (SELECT id FROM site_content_history ORDER BY id DESC LIMIT 30)").run();
  }
  await db.prepare(`INSERT INTO site_content (id, payload, updated_at, updated_by)
    VALUES (1, ?, CURRENT_TIMESTAMP, ?)
    ON CONFLICT(id) DO UPDATE SET payload = excluded.payload, updated_at = CURRENT_TIMESTAMP, updated_by = excluded.updated_by`)
    .bind(JSON.stringify(clean), editor).run();
  return clean;
}

export async function listSiteContentHistory(): Promise<ContentRevision[]> {
  const db = await ensureStorage();
  const result = await db.prepare("SELECT id, created_at, updated_by FROM site_content_history ORDER BY id DESC LIMIT 30").all<{ id: number; created_at: string; updated_by: string }>();
  return (result.results || []).map((row) => ({ id: String(row.id), createdAt: row.created_at, updatedBy: row.updated_by }));
}

export async function restoreSiteContentHistory(id: string, email: string): Promise<SiteContent> {
  if (!/^\d+$/.test(id)) throw new Error("invalid revision");
  const db = await ensureStorage();
  const row = await db.prepare("SELECT payload FROM site_content_history WHERE id = ?").bind(Number(id)).first<{ payload: string }>();
  if (!row?.payload) throw new Error("revision not found");
  return saveSiteContent(JSON.parse(row.payload), email);
}
