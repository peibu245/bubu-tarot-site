...
export default function GuestbookManager() {
  ...
  return <CollapsiblePanel label="12 / GUESTBOOK" title="留言板管理"><section className="editor-section guestbook-manager"><div className="editor-title"><div><span>12 / GUESTBOOK</span><h2>留言板管理</h2></div></div><p className="manager-hint">公开留言会先经过基础过滤。这里只显示最新 80 条；删除后无法恢复。</p>{status && <p className="editor-empty">{status}</p>}<div className="manager-entries">{entries.map((entry) => <article key={entry.id}><div><b>{entry.nickname}</b><time>{new Date(entry.createdAt).toLocaleString("zh-CN")}</time><button type="button" onClick={() => remove(entry.id)}>删除</button></div><p>{entry.message}</p></article>)}</div></section></CollapsiblePanel>;
}
